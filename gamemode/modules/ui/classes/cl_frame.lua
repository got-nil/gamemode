...
function PANEL:Init()
    ...
    self:GNIL():Background(GNIL.Ui.Colors["Nav"], 8):FadeIn()
    self.nav = GNIL.Ui.Create("DPanel", self)
    self.nav:Stick(TOP)
    self.nav:ClearPaint():Background(Color(35, 35, 35), 8)
    self.close = GNIL.Ui.Create("DButton", self.nav)
    self.close:Stick(RIGHT)
    self.close:SetText("")
    self.close:SetWide(self.nav:GetTall())
    self.close:ClearPaint():ClearPaint():Background(Color(35, 35, 35), 8):Text("✕", GNIL.Ui.ensureFontExists("GNIL_MontserratM", 15)):FadeHover(GNIL.Ui.Colors["Red"], 8, 8)
